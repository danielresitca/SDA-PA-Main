...
class ModernApp:
    ...
    def process_invoice(self):
        ...
        loading_window.title("Procesare...")
        ...
        # Loading content
        loading_frame = tk.Frame(loading_window, bg=self.colors['bg'])
        loading_frame.pack(expand=True, fill='both', padx=30, pady=30)

        # Spinner canvas
        spinner_canvas = tk.Canvas(loading_frame, width=60, height=60,
                                   bg=self.colors['bg'], highlightthickness=0)
        spinner_canvas.pack(pady=(0, 15))

        # Status label
        status_label = tk.Label(loading_frame,
                                text='Se proceseaza factura...',
                                font=self.fonts['body'],
                                bg=self.colors['bg'],
                                fg=self.colors['text'])
        status_label.pack()
        ...
        def draw_spinner():
            ...
            for i in range(8):
                angle = spinner_angle[0] + i * 45
                x = 30 + 20 * math.cos(math.radians(angle))
                y = 30 + 20 * math.sin(math.radians(angle))
                alpha = 1 - (i / 8)
                color = self._blend_color(self.colors['accent'], self.colors['bg'], alpha)
                spinner_canvas.create_oval(x - 3, y - 3, x + 3, y + 3, fill=color, outline='')
            spinner_angle[0] = (spinner_angle[0] + 15) % 360
